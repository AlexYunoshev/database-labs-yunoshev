...
{
    public partial class DeleteStorehouseFields : MaterialForm
    {
        readonly MaterialSkin.MaterialSkinManager materialSkinManager;

        public DeleteStorehouseFields()
        {
            InitializeComponent();
        ...
        private void ButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                Models.StorehouseField.MedicationsName = TextBoxName.Text;
                if (CheckBoxSpecificId.Checked == true)
                {
                    Models.StorehouseField.id1 = Convert.ToInt32(NumericSpecificId.Value);
                    Models.StorehouseField.id2 = -1;
                }
                else if (CheckBoxRangeId.Checked == true)
                {
                    Models.StorehouseField.id1 = Convert.ToInt32(NumericFrom.Value);
                    Models.StorehouseField.id2 = Convert.ToInt32(NumericTo.Value);
                }
                else
                {
                    Models.StorehouseField.id1 = -1;
                    Models.StorehouseField.id2 = -1;
                }
                this.Close();

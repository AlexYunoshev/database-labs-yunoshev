...
{
    public partial class DeletePrescriptions : MaterialForm
    {
        readonly MaterialSkin.MaterialSkinManager materialSkinManager;

        public DeletePrescriptions()
        {
            InitializeComponent();
        ...
        private void ButtonDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                Models.Prescriptions.patientName = TextBoxName.Text;
                if (CheckBoxSpecificId.Checked == true)
                {
                    Models.Prescriptions.id1 = Convert.ToInt32(NumericSpecificId.Value);
                    Models.Prescriptions.id2 = -1;
                }
                else if (CheckBoxRangeId.Checked == true)
                {
                    Models.Prescriptions.id1 = Convert.ToInt32(NumericFrom.Value);
                    Models.Prescriptions.id2 = Convert.ToInt32(NumericTo.Value);
                }
                else
                {
                    Models.Prescriptions.id1 = -1;
                    Models.Prescriptions.id2 = -1;
                }
                this.Close();
